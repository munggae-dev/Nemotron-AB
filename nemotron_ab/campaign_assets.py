"""A/B 변형 이미지 자산: 스테이징 업로드, job별 승격(guard path), LLM용 data URL 해석."""
...
def _resolve_image_max_dim(explicit: Optional[int]) -> int:
    ...
    if explicit is not None:
        try:
            ...
            pass
    raw = os.environ.get("LLM_IMAGE_MAX_DIM", "").strip()
    if raw:
        try:
            return int(raw)
        except ValueError:
...
def _downscale_image_bytes(data: bytes, max_dim: int, mime_hint: str) -> Tuple[bytes, str]:
    ...
    호출부의 멀티모달 전송이 중단되지 않도록 한다.
    """
    if max_dim <= 0 or not data:
...
def image_ref_to_data_url(ref: Dict[str, Any], max_dim: Optional[int] = None) -> str:
    ...
    None이면 환경변수 LLM_IMAGE_MAX_DIM을 사용하고, 그것도 없으면 기본값 512.
    0 이하이면 비활성화.
    """
    t = str(ref.get("type", ""))
    v = str(ref.get("value", "")).strip()
    eff_max = _resolve_image_max_dim(max_dim)
    if t == "path":
        ...
        data, mime = _downscale_image_bytes(data, eff_max, mime)
