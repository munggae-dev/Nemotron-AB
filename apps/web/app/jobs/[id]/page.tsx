...
function JobProgressPanel({ progress }: { progress: JobProgress }) {
  ...
  const pct = indeterminate ? 0 : Math.min(100, Math.max(0, progress.percent));
...
export default function JobDetailPage() {
  ...
  const copyA = typeof campaign?.copy_a === "string" ? campaign.copy_a : "";
  const copyB = typeof campaign?.copy_b === "string" ? campaign.copy_b : "";
      ...
      {job?.status === "completed" && overall && report && (
        <>
              ...
              <div className="report-confidence-heading">
                ...
                <details className="report-confidence-help">
                  ...
                  <div className="report-confidence-explain">
                    ...
                    <p>
                      <strong>해석:</strong> 숫자가 클수록 많은 페르소나에서 두 카피의 점수가 서로{" "}
          ...
          <div className="report-section-card">
            <h3>연령 버킷별 요약</h3>
            <p className="section-sub">각 버킷에서 계산된 평균 점수 비중을 시각화했습니다.</p>
            ...
            <div className="bucket-bars">
              {Object.entries(summaryByBucket).map(([bucket, s]) => {
                if (!s || typeof s.count !== "number" || s.count <= 0) return null;
                const max = Math.max(s.avg_score.A, s.avg_score.B, 1e-6);
                const wa = Math.round((s.avg_score.A / max) * 100);
                const wb = 100 - wa;
                ...
                return (
                  <div key={bucket} className="bucket-bar-row">
                    ...
                    <div className="bucket-bar-track">
                      <div className="bucket-bar-seg bucket-bar-seg--a" style={{ width: `${wa}%` }}>
                        A
                      </div>
                      <div className="bucket-bar-seg bucket-bar-seg--b" style={{ width: `${wb}%` }}>
                        B
                      </div>
                    </div>
